  ...
  <nav>
    <v-app-bar app class="" color="#ddccff" height="65px">
      ...
      <div flat color="#E6E6FA" class="mt-1 ml-3">
        <v-toolbar-title
          ...
          @click="
            $router.push({
              name: 'Dashboard',
              params: { username: userBio[0].email },
            });
            changeCurrentUrl();
          "
        >
      ...
      <v-tabs v-if="!userBio" right icons-and-text>
        ...
        <v-tab disabled to="/signup" @click="changeCurrentUrl">
              ...
              <v-menu :nudge-width="200" offset-y>
                <template v-slot:activator="{ on, attrs }">
                  <v-avatar size="60" dark v-bind="attrs" v-on="on">
                    <img
                      :src="require(`../assets/users/${userBio[0].image}`)"
                      :alt="`../assets/users/${userBio[0].image}`"
                    />
                  </v-avatar>
                </template>
                ...
                <v-card class="mt-0">
                  ...
                  <v-divider></v-divider>
                  <v-list dense>
                    <v-list-item-group v-model="selectedUserItem">
                      <v-list-item
                        v-for="profile in profiles"
                        :key="profile.title"
                        @click="userNavigation(profile.title)"
                      >
